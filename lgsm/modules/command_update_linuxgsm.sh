	...
	if [ -d "${modulesdir}" ]; then
		...
		for functionfile in *; do
			...
			if [ $? != 0 ]; then
				...
				# compare file
				if [ "${remotereponame}" == "GitHub" ]; then
					function_file_diff=$(diff "${modulesdir}/${functionfile}" <(curl --connect-timeout 10 -s "https://raw.githubusercontent.com/${githubuser}/${githubrepo}/${githubbranch}/${github_file_url_dir}/${functionfile}"))
				else
					function_file_diff=$(diff "${modulesdir}/${functionfile}" <(curl --connect-timeout 10 -s "https://bitbucket.org/${githubuser}/${githubrepo}/raw/${githubbranch}/${github_file_url_dir}/${functionfile}"))
				fi
